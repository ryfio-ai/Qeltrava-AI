...
export const PartnerStrip = () => (
  <section className="py-12 bg-[var(--color-bg-light)] border-t border-[var(--color-border-soft)] select-none">
    <div className="max-w-7xl mx-auto px-6 md:px-12">
      <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--color-text-main)]/50 text-center mb-8 font-mono">
        ...
        {badges.map((b) => (
          <div
            ...
            className={`flex flex-col items-center text-center px-5 py-3 rounded-xl border text-sm font-bold leading-snug min-w-[120px] ${
              b.muted
                ? 'border-[var(--color-border-soft)] text-[var(--color-text-main)]/40 italic'
                : 'border-[var(--color-accent)]/30 text-[var(--color-accent)]'
            }`}
          >
            <span>{b.label}</span>
            <span className={`text-[10px] font-normal mt-0.5 ${b.muted ? 'text-[var(--color-text-main)]/30' : 'text-[var(--color-text-main)]/50'}`}>
              {b.note}
            </span>
          </div>
        ))}
